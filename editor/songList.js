...
export default async function loadSongs() {
  ...
  try {
    ...
    const url = 'https://UnderlineGH.github.io/html5bytebeat-forksBETA/editor/songs.json';

    const res = await fetch(url);
    const songs = await res.json();
    const localBase = `${window.location.origin}${window.location.pathname}`;
    const origBase = 'https://UnderlineGH.github.io/html5bytebeat-forksBETA/html5bytebeat.html';
